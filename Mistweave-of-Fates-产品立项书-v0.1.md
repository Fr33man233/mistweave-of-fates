...
> 当前产品方向已由[平行廷根 AI Keeper RPG：产品重定义与全栈预审](./docs/superpowers/specs/2026-08-20-parallel-tingen-ai-keeper-product-redefinition.md)更新。后续系统细化见[MVP 系统设计基线 v0.1](./Mistweave-of-Fates-MVP系统设计-v0.1.md)，旧 V0.3 设计见[真实调查闭环版全栈预审与设计规格](./docs/superpowers/specs/2026-08-20-v03-investigation-closure-design.md)。这些文件是目标设计或历史证据而非新产品完成证明；当前成熟度与发布门以[版本定义与发布门](./docs/版本定义与发布门.md)及较新的已确认规格为准。
...
> 当前结论：**V0.2 已作为“首次晋升规则系统验证版”发布：机械流程、自动化验收、浏览器复核和 Pages 部署成立，但日常调查反馈、主动材料获取、事件内能力价值及完整失败/恢复路径尚未形成，因此当前未达到 MVP，也未进入 MVP 后内测或 V1.0 公测开发门。**
...
### 11.3 V1.0 公测前 UI 资源策略

V0.2 现有的工业神秘风格、简洁布局和低操作负担作为 V1.0 公测前各迭代的默认视觉基线。V1.0 公测前不进行高成本美术采购、大规模视觉重制或仅为装饰增加复杂组件；UI 投入优先保证信息清楚、操作简便、响应式可用和核心流程可完成。V1.0 公测发布后，再根据公开反馈单独寻找美术资源并立项评估品牌视觉、原创资产和系统性 UI 升级，避免在玩法尚未稳定时产生重复设计与返工；该评估不改变 V2.0 才是正式上线版的定义。
...
- 一个原创的小型恐怖调查区域。
- 单个标志性案件目标为 10–15 分钟或更短；V1.0 公测以四职业、四条可玩路径且每条只开放首次晋升阶位为规划基线，至少提供 10 条公共标志性案件、每职业 1 条标志性案件、每路径 1 条获得/晋升标志性案件，共 18 条完整手工案件。
- 另提供 6–8 套可重复调查语法；每个职业通过透镜向兼容案件提供至少 3 个专属入口或解法，每条路径提供至少 3 个跨案件能力互动点。新增职业或路径主要扩展透镜和至多 1 条标志性案件，不复制职业×路径组合剧情。
- 调查内容采用“案件内核/调查语法 + 职业透镜 + 路径透镜 + 城市状态”：案件实例创建时由规则引擎固化真相，职业和路径改变入口、可见信息、方法和代价，AI 只解释行动并叙述已提交结果。
...
“已发布”“规则原型”“系统验证版”“MVP”“内测版”“公测版”和“正式上线版”不得互换使用。当前统一结论为：V0.1 是连续调查规则原型，V0.2 是首次晋升规则系统验证版；两者为 MVP 建立了确定性规则、存档和部署基础，但没有完成本节所要求的自由行动、AI 叙事、持续世界、完整调查反馈和长局验证，因此 **MVP 尚未达到**。

MVP 通过后的 V0.x 版本属于小范围内测；V1.0 不是“比 V0.2 内容更多”的别名，而是第一个必须支持公网访问的原创单人公测版，后续 V1.x 继续用于公开测试与反馈。V2.0 暂定为正式上线版，最低内容门为 15 个可玩职业和 12 条具有完整可玩内容的路径，但完整成长深度与验收数量仍需独立预审。各阶段完整发布门、当前缺口及非默认范围以[版本定义与发布门](./docs/版本定义与发布门.md)为准。
...
## 22. 2026-08-20 产品重定义（当前权威摘要）

本节与[平行廷根 AI Keeper RPG：产品重定义与全栈预审](./docs/superpowers/specs/2026-08-20-parallel-tingen-ai-keeper-product-redefinition.md)共同覆盖本文件中与下列结论冲突的旧建议；旧内容继续保留为立项历史，不被删除或伪装成当前实现。

- 当前世界是没有原著主线与核心人物的平行廷根；采用《诡秘之主》世界观和力量途径作为唯一完整内容包，底层保持 `WorldPack` 可替换。
- 普通人的世界危险度高于原著叙事中的常态，通过有前兆、有来源、可规避和可复盘的事件压力推动玩家主动接触超凡力量。
- 人类规则底座严格更换为 COC 7e 八属性、技能、难度、奖励/惩罚骰、幸运、孤注一掷、HP、重伤、濒死、SAN 和技能成长。
- COC MP 替换为灵性；SAN 合并污染表现并显示为 `SAN：45/70（轻度污染）`；扮演进度包含失控表现并显示为 `扮演进度：初步接受（轻微失控）`。
- 序列晋升提升部分 COC 基础属性、灵性上限和途径能力；能力首轮优先用权限、自动成功、奖励/惩罚骰和难度变化验证，但不被限制为骰子修正。
- V1.0 四条正式途径为占卜家→小丑、观众→读心者、不眠者→午夜诗人、猎人→挑衅者，至少完整实现序列 9–8、扮演和多个事件内能力应用。
- AI Keeper 主持玩家场景；世界 AI 采用 B+ 权限组合合法危险与机会，临时内容必须经永久化门禁才能成为世界事实；规则引擎继续拥有唯一裁决权。
- 第一份可玩新产品切片即接入真实模型；首个供应商固定为 DeepSeek `deepseek-v4-flash`，只通过本地服务端网关发送最小可见场景切片，模型桩不能替代体验证据。
- 继承与复活不在 V1.0 前实现；最终案件数量等待真实 Keeper/世界 AI 纵向切片后重新预审。
- 未取得相应授权前只允许私人或封闭测试，不进入 V1.0 公网公测。
