# Mistweave of Fates（灰雾织命）：MVP 系统设计基线

> 版本：v0.1  
> 状态：MVP 目标设计基线；当前实现尚未达到该目标
> 上位文档：[产品立项书](./Mistweave-of-Fates-产品立项书-v0.1.md)  
> 本文目的：把已确认的产品方向转化为可实现、可验证、可在试玩后修改的系统合同。
> 当前成熟度与发布门：[版本定义与发布门](./docs/版本定义与发布门.md)；后续已确认规则优先于本文旧草案。
...
- 不是公测版或正式上线版；MVP 通过后先进入小范围 V0.x 内测，V1.x 才是必须支持公网访问的公测，V2.0 暂定为正式上线。
...
| **本版实现** | 单城市、本地存档、D100、4 个首发职业入口、日常/城市/超自然并行任务、两条线索链、伤势与法律后果 | 构成可测试的核心闭环 |
...
| 内容生产 | 4 个首发职业分别写任务 | 标签 + 共享模板 + 职业变量插槽 | 避免职业内容分叉 | 统计模板复用率和职业覆盖率 |
...
### 7.1 MVP 首发职业池（后续确认覆盖旧草案）

药房学徒、记者、警探、码头工人。

医生、药剂师、商人、工匠、酒吧老板、考古学家、牧师、小偷、水手、学者、艺术家、马车夫等旧草案职业全部延期；若要恢复，必须在后续版本预审中证明内容复用、经济和任务成本可控。裁缝、糕点师、冒险家等专精/背景归类也随之延期。
...
这些差异统一保存为 `OccupationLens`：职业透镜可以增加入口、联系人、权限、义务、判定方法和失败代价，但不能替换案件真相或直接授予线索。未来恢复旧草案职业时，默认新增一套透镜和至多一条职业标志性案件，不要求为每条超凡路径制作独立职业剧情。
...
调查内容分为五层：案件内核保存不可变真相；调查语法保存可实例化的结构和变量槽；职业透镜保存世俗入口与解法；路径透镜保存超凡方法、消耗和风险；城市状态提供 NPC、势力、地点与并发复杂因素。调查语法生成实例时必须以种子固化人物、地点、动机、事实和时钟，之后 AI 无权重新选择真相。
...
- 成功、部分成功、失败、拒绝和忽略的后果。
- 消耗时间、金钱、关系、伤势或法律风险。
- 可绑定的证据、传闻、物品槽位及其允许揭示的既有事实 ID；模板和 AI 都不能临时创造关键真相。
- 对任务时钟、城市时钟和 NPC 目标的影响。

### 9.3 MVP 内容预算（暂定）

- 1 条代表性标志案件和 1 套最小调查语法，用于验证模块组合、自由行动和规则收束；它们不是固定台词和固定场景顺序。
- 至少 1 个会分阶段演变并影响后续选择的城市事件。
- 2 条可并行调查的力量线索链。
- 至少 3 个完全不涉及超自然的任务。
- 4 个城区、约 12 个关键地点、12–18 名有状态的命名 NPC。
- 其余人群使用角色模板，不建立逐人长期模拟。

V1.0 公测再扩展为 18 个手工标志性案件和 6–8 套调查语法；V2.0 正式上线的 15 职业与 12 条完整路径不属于 MVP 内容预算。
...
- 4 个首发职业分别运行 30 个游戏日的经济模拟，比较净可用价值和被迫停工率。
...
- 完成原创或已获授权的公开内容边界检查。
- 输出：可下载的封闭测试版或合规演示版。通过 MVP 后仍先进入 V0.x 小范围内测，不因存在公网演示地址自动成为公测。
...
- 普通人开局，两条隐晦力量线索可同时调查。
- 日常、职业、城市和超自然内容并行。
- 4 个首发职业通过标签和共享模板复用内容。
- 调查内容采用案件内核/语法、职业透镜、路径透镜和城市状态组合；新增职业或路径不复制完整案件。
...
- MVP 使用 1 套最小调查语法和至少 1 个阶段性城市事件；V1.0 公测再扩展为 6–8 套调查语法。
- 常规回合单次模型调用目标。
- 首版低序列可玩深度建议为 9–7。

### 20.3 下一份设计产物

在没有新的重大选择前，下一步应建立[规则与内容数据合同 v0.1](./Mistweave-of-Fates-V0.1规则与内容数据合同.md)，具体定义角色卡、职业卡、事件模板、线索、判定请求和已提交事件的字段与示例，并用少量样例验证它们能共同完成一个回合。
...
- **v0.1（2026-08-19）**：补充已实现的下游合同、试玩样例、验收发布记录和 V0.2 设计入口；原有范围与规则不变。
