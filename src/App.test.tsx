...
describe('V0.2 首次晋升界面', () => {
  ...
  it('要求二次确认后才提交首次服药', async () => {
    ...
    expect(screen.getByRole('button', { name: '使用痕迹感知' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: '调查猎犬线索' })).not.toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: '使用痕迹感知' }));
  ...
  it('恢复灾难失败存档时禁用永久失效角色卡', async () => {
    ...
    game = prepareRitual(advanceTrack(game, 'hound'), 'hound', 'safe');
