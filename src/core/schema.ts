...
export const characterSchema = z.object({
  characterId: z.string().min(1), status: z.enum(['active', 'deceased', 'retired']), name: z.string().trim().min(1), gender: z.enum(['female', 'male', 'nonbinary', 'unspecified']), occupationId: z.string().min(1), initialIntent: z.string().min(1),
